...
## Keyword-Ranking Sync & UX (29.04.2026)
- **DataForSEO Bug behoben**: `Keyword_ID` wurde als Array `["recXYZ"]` in Airtable geschrieben, das Feld akzeptiert aber nur einen plain String. Fix: `upsertKeywordRankingHistory` in `airtable.ts` schreibt nun `Keyword_ID: item._kwId` (String, kein Array).
- **Kein-Ranking-Sonderwert `101`**: Keywords die nicht in den Top 100 ranken, erhalten `Ranking: 101` statt still übersprungen zu werden. `sync-performance.ts` filtert `null`-Rankings nicht mehr heraus.
- **Dedup-Logik bestätigt**: Vor jedem DataForSEO-API-Call prüft `getExistingRankingDates()` ob für die aktuelle Woche bereits Einträge vorliegen. Keywords mit vorhandenem Eintrag werden übersprungen. Bei manuellem Sync (`force=true`) wird der Check umgangen.
- **Debug-Endpoint ausgebaut** (`/api/admin/debug?url=...&testWrite=true`): Vollständige End-to-End-Diagnose in 7 Schritten (Credentials, Keywords, Dedup, Domain-Extraktion, Live-API-Call, Would-Upsert, Actual-Upsert).

## UI & Visualisierung (URL Detail & Monitoring)
- **Erweiterte Detailansicht**: Die `UrlDetail` Komponente zeigt nun zwei separate Charts:
  1. **URL-Performance**: Zeitverlauf von Klicks und Sistrix VI. ReferenceLine-Marker für Erstellung/Optimierung: gestrichelt (`strokeDasharray="4 3"`), dünn (`strokeWidth=1`), Flag-Badge oben am Linienstart.
  2. **Keyword-Rankings**: Individuelle Kurven für alle zugeordneten Keywords mit **logarithmischer Y-Achse** (Fokus auf Positionen 1–20, Ticks: 1·3·5·10·20·50·>100). Datentransformation via `Math.log(rank)`, Tooltip zeigt Originalwert. Datenpunkte mit `101` als grauer gestrichelter Kreis.
- **Keywords-Karte neu gestaltet**: Tabellarisches Zeilen-Layout (Keyword-Name + Main-Pill links, aktuelles Ranking rechts). Farbkodierung: ≤10 grün · ≤20 lime · ≤50 gelb · ≤100 grau · >100 orange · kein Eintrag `–`. ScrollArea bei vielen Keywords.
